...
export default function Register() {
  ...
  // 🚀 Render सर्व्हर लोडिंग स्क्रीन लपवण्यासाठी स्टेट
  const [isWakingUp, setIsWakingUp] = useState(false);

  // 🟢 पेज लोड झाल्यावरच बॅकएंडला जागा करण्यासाठी पिंग पाठवा
  useEffect(() => {
    fetch("https://footpryx-backend.onrender.com/", { method: "GET", mode: "no-cors" })
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage("Registration failed. Try using a different email.");
      }
    } catch (error) {
      setIsWakingUp(false); // एरर आल्यास पॉपअप बंद करा
  ...
  // 🚀 अपडेटेड Google Register (Render चे लोडिंग पेज लपवण्यासाठी)
  const handleGoogleRegister = async () => {
    setIsWakingUp(true); // लोडिंग पॉपअप ऑन करा

    try {
      // सर्व्हरला जागा करण्यासाठी पिंग रिक्वेस्ट
      await fetch("https://footpryx-backend.onrender.com/", { method: "GET", mode: "no-cors" });
    } catch (err) {
      console.log("Wake up ping sent");
    }

    // थोड्या वेळाने गुगल लॉगिन पेजवर पाठवा
    setTimeout(() => {
        ...
        <div className="fixed inset-0 bg-black/85 z-50 flex flex-col items-center justify-center p-6 text-center space-y-4 backdrop-blur-md animate-fadeIn">
          ...
          <div className="space-y-1.5 max-w-sm">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Server is waking up from sleep mode. Please wait a moment for initial connection.
            </p>
          </div>
        </div>
      )}
