...
export default function Sidebar({ activeMenu, setActiveMenu }) {
  return (
    // 🛠️ इथे आपण font-sans वापरून फॉन्ट स्लीक केला आहे आणि tracking-wider काढून बारीक डिझाईन दिली आहे
    <aside className="w-64 border-r border-emerald-950/60 bg-black flex flex-col justify-between flex-shrink-0 h-full font-sans antialiased text-emerald-500">
      <div className="space-y-6">
        ...
        <nav className="px-4 space-y-2">
          <button 
            onClick={() => setActiveMenu && setActiveMenu("search")}
          ...
          <button 
            onClick={() => setActiveMenu && setActiveMenu("investigations")}
          ...
          <button 
            onClick={() => setActiveMenu && setActiveMenu("history")}
          ...
          <button 
            onClick={() => setActiveMenu && setActiveMenu("settings")}
            className={`w-full flex items-center gap-3 px-4 py-2.5 rounded border text-xs font-bold uppercase transition-all text-left ${
              activeMenu === "settings" 
                ? "bg-emerald-950/20 text-[#a3e635] border-emerald-800/40" 
                : "text-emerald-600 border-transparent hover:text-emerald-400"
        ...
        {/* Bottom Core Monitor Box from your screenshot */}
        <div className="px-4">
