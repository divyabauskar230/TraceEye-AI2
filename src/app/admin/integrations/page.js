...
export default function AdminIntegrationsPage() {
  ...
  const integrationsList = [
    { key: "shodan", name: "Shodan IoT Scanner", desc: "Global IoT device telemetry and open port mapping.", statusKey: "shodan" },
    { key: "hibp", name: "HaveIBeenPwned API", desc: "Enterprise data breach and leaked credentials feed.", statusKey: "hibp" },
    { key: "virustotal", name: "VirusTotal Security API", desc: "Malware hash checking and domain threat analysis.", statusKey: "virustotal" },
    { key: "telegram", name: "Telegram Threat Bot", desc: "Real-time darkweb breach alerts sent to secure channels.", statusKey: "telegram" },
  ];
    ...
    <div className="min-h-screen bg-[#02060f] text-white flex font-sans selection:bg-lime-500 selection:text-black">
      ...
      <aside className="w-64 bg-[#050914] border-r border-gray-800/80 flex flex-col justify-between p-5 select-none hidden lg:flex shrink-0">
        <div className="space-y-6">
          ...
          <nav className="space-y-1 text-xs font-medium">
            ...
            <Link href="/admin/integrations" className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-lime-500/10 text-lime-400 border border-lime-500/20 font-bold transition">
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        <header className="h-20 border-b border-gray-800/80 px-8 flex items-center justify-between bg-[#040812]/80 backdrop-blur-md sticky top-0 z-30">
          <div>
            <h2 className="text-xl font-black text-white tracking-tight">API & Service Integrations</h2>
            <p className="text-xs text-gray-400">Connect third-party security databases and telemetry providers.</p>
          </div>
        ...
        <div className="p-8 space-y-6 max-w-[1600px] w-full mx-auto">
          <div className="bg-[#060a17] border border-gray-800/80 rounded-3xl p-6 shadow-lg space-y-6">
            <h3 className="text-sm font-bold text-white">Connected Ecosystem</h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {integrationsList.map((item) => {
                const isConn = connectedMap[item.key];
                return (
                  <div key={item.key} className="bg-black/50 border border-gray-800/80 p-5 rounded-2xl flex items-center justify-between">
                    <div className="space-y-1">
                      <h4 className="text-xs font-bold text-white">{item.name}</h4>
                      <p className="text-[10px] text-gray-400 font-sans">{item.desc}</p>
                      <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-mono font-bold mt-1 ${isConn ? 'bg-emerald-950 text-emerald-400 border border-emerald-500/30' : 'bg-gray-900 text-gray-400 border border-gray-800'}`}>
                        {isConn ? 'Connected' : 'Disconnected'}
                      </span>
                    </div>
                    <button 
                      onClick={() => toggleIntegration(item.key)}
                      className={`px-4 py-2 rounded-xl text-xs font-extrabold transition cursor-pointer ${isConn ? 'bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30' : 'bg-lime-500 text-black hover:bg-lime-400'}`}
                    >
