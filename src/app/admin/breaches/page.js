...
export default function AdminBreachesPage() {
  ...
  // State for breaches list and modals
  const [breachesList, setBreachesList] = useState([
    { id: "BRC-101", target: "Enterprise Global DB", vector: "CRED_DUMP", rows: "4.2M Rows", severity: "Critical", status: "Isolated" },
    { id: "BRC-102", target: "FinTech Auth Server", vector: "SESSION_HIJACK", rows: "850K Rows", severity: "High", status: "Active" },
    { id: "BRC-103", target: "Cloud Storage Bucket", vector: "API_EXPOSE", rows: "1.2M Rows", severity: "Medium", status: "Mitigated" },
    { id: "BRC-104", target: "Internal Git Repository", vector: "SOURCE_LEAK", rows: "320K Rows", severity: "High", status: "Monitoring" },
  ]);
  ...
  const handleTriggerScan = () => {
    const randomId = "BRC-" + Math.floor(110 + Math.random() * 800);
    const newEntry = {
      id: randomId,
      target: "External Partner Gateway",
    ...
    setBreachesList([newEntry, ...breachesList]);
    setSuccessMsg("Emergency security scan completed! New breach isolated.");
  ...
  return (
    <div className="min-h-screen bg-[#02060f] text-white flex font-sans selection:bg-lime-500 selection:text-black relative">
      ...
      {/* 🔍 ANALYZE MODAL */}
      {isAnalyzeModalOpen && selectedBreach && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#0b0f19] border border-red-500/40 rounded-3xl p-6 md:p-8 max-w-lg w-full space-y-6 shadow-2xl relative">
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <div>
                <span className="text-xs font-mono text-red-400 font-bold">{selectedBreach.id} // BREACH FORENSICS</span>
                <h3 className="text-lg font-extrabold text-white">{selectedBreach.target}</h3>
              </div>
              <button 
            ...
            <div className="space-y-4 text-xs font-mono text-slate-300">
              <div className="flex justify-between bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                ...
                <span className="text-white font-bold">{selectedBreach.vector}</span>
              </div>
              <div className="flex justify-between bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <span className="text-slate-500">Exposed Volume:</span>
                <span className="text-lime-400 font-bold">{selectedBreach.rows}</span>
              </div>
              <div className="flex justify-between bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <span className="text-slate-500">Severity Rating:</span>
                <span className="text-red-400 font-bold">{selectedBreach.severity}</span>
              </div>
              <div className="flex justify-between bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <span className="text-slate-500">Containment Status:</span>
                <span className="text-emerald-400 font-bold">{selectedBreach.status}</span>
              </div>
            </div>
      ...
      <aside className="w-64 bg-[#050914] border-r border-gray-800/80 flex flex-col justify-between p-5 select-none hidden lg:flex shrink-0">
        <div className="space-y-6">
          ...
          <nav className="space-y-1 text-xs font-medium">
            ...
            <Link href="/admin/breaches" className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-lime-500/10 text-lime-400 border border-lime-500/20 font-bold transition">
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        <header className="h-20 border-b border-gray-800/80 px-8 flex items-center justify-between bg-[#040812]/80 backdrop-blur-md sticky top-0 z-30">
          <div>
            <h2 className="text-xl font-black text-white tracking-tight">Data Breaches & Dumps</h2>
            <p className="text-xs text-gray-400">Monitor dark web breach streams and enterprise threat vectors.</p>
          </div>
        ...
        <div className="p-8 space-y-6 max-w-[1600px] w-full mx-auto">
          <div className="bg-[#060a17] border border-gray-800/80 rounded-3xl p-6 shadow-lg space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-white">Active Darkweb Breach Streams ({breachesList.length} Total)</h3>
              <button 
                onClick={handleTriggerScan}
                className="bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 font-extrabold px-4 py-2 rounded-xl text-xs transition cursor-pointer flex items-center gap-2"
              >
                <span>⚡</span> Trigger Emergency Scan
              </button>
            </div>
              ...
              <table className="w-full text-left text-xs font-mono">
                <thead>
                  <tr className="border-b border-gray-800 text-gray-500 uppercase text-[10px]">
                    <th className="pb-3">Breach ID</th>
                    <th className="pb-3">Target Entity</th>
                    <th className="pb-3">Vector Type</th>
                    <th className="pb-3">Exposed Rows</th>
                ...
                <tbody className="divide-y divide-gray-900/80">
                  {breachesList.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-900/40 transition">
                      ...
                      <td className="py-4 text-gray-300">{item.vector}</td>
                      <td className="py-4 text-lime-400 font-bold">{item.rows}</td>
                      <td className="py-4">
