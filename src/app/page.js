...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (emailParam) {
        ...
        setUser(userData);
        localStorage.setItem("traceeye_user", JSON.stringify(userData));
        localStorage.setItem("traceeye_credits", "2");
        setCredits(2);
      } else {
      ...
      const savedCredits = localStorage.getItem("traceeye_credits");
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("traceeye_credits");
    setUser(null);
  ...
  const handleSearch = async (e) => {
    ...
    // पूर्ण नंबर तयार करणे (जर फोन असेल तर कंट्री कोड + नंबर)
    ...
    try {
      const scanRes = await fetch("http://127.0.0.1:8000/api/osint/scan", {
        method: "POST",
      ...
      const existingHistory = JSON.parse(localStorage.getItem("traceeye_history") || "[]");
      localStorage.setItem("traceeye_history", JSON.stringify([newLog, ...existingHistory]));
    }
  };
        ...
        <div className="w-full max-w-2xl mx-auto space-y-8 my-auto py-10">
          ...
          <div className="text-center space-y-2">
            ...
            <h2 className="text-3xl font-bold tracking-tight text-white">
              {user ? `Hello, ${user.name.split(" ")[0]}.` : "espectrosint OSINT"}
            </h2>
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div className="relative flex items-center bg-[#0b0f19] border border-slate-900 rounded-2xl p-1 shadow-xl">
              <div className="pl-4 text-slate-500 flex items-center gap-2">
                ...
                {/* 🌍 जर फोन टॅब असेल तर जगातील सर्व देशांचे कोड्स आणि झेंडे असलेला ड्रॉपडाऊन दिसेल */}
                {activeTab === "phone" && (
                  <select
